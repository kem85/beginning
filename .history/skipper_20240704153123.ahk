...
MButton::
{
    ...
    file := FileOpen("C:\Users\kem7\Documents\GitHub\beginning\link.txt", "w") 
    file.write("")
    file.close()
    clip := string(A_Clipboard)
    FileAppend clip, "C:\Users\kem7\Documents\GitHub\beginning\link.txt"
    all1 := pythonn . path . clip
    link := runwait(all1)
    sleep 80
    buff :=FileRead("C:\Users\kem7\Documents\GitHub\beginning\link.txt")
    A_Clipboard := buff
    send "^v"
    Send('{Enter}')
    loop 5
