    ...
    for(int z = 0 ; z < countr ; z++)
    {
        for(int i = 0 ; i < num.length() ; i ++)
        {
            ...
            else if(num[i] == '*' || num[i] == '/')
            {
                ...
                ladd = reverse(ladd);
                string errorer= (num[i] == '*' ? to_string(stof(ladd)*stof(radd)) : (radd != "0") ? to_string(stof(ladd)/stof(radd)) : "can't divide by zero");
                cal = (num[i] == '*' ? stof(ladd)*stof(radd) : (radd != "0") ? stof(ladd)/stof(radd) : 404);
                num.erase(start,(end-start)+1);
                if(start!=0)
                {
