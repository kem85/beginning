...
def openNewWindow():
    global windowIsOpen
    if not windowIsOpen.get():  # Check if window is not already open
        ...
        newwindow.geometry(f'{windowWidth}x{windowHeight}+{centerX}+{centerY}')
        ...
        newWindow.protocol("WM_DELETE_WINDOW", lambda: (windowIsOpen.set(False) , newWindow.destroy()))
    else:
