...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Catagory = tb.Checkbutton(addacc,width=20,text="Catagory",onvalue=1,offvalue=0,font=('Helvetica', 18)).grid(column=0,row=4)
        Submit = tb.Button(addacc,width=20,text="Submit").grid(column=1,row=4,pady=10,padx=10)
def EDIT():
