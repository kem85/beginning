...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        addwindow.geometry(f'{windowWidth}x{windowHeight}+{centerX}+{centerY}')
        root.resizable(False, False)
        addwindow.protocol("WM_DELETE_WINDOW", lambda: (addwindow.set(False) , addwindow.destroy()))
        addcc = tb.Button(addwindow, text='Add',takefocus=False,width=5,style=PRIMARY)
        addcc.pack(side='left', anchor='e')
def EDIT():
...
edit = tb.Button(root, text='Edit',takefocus=False,width=5,style=PRIMARY,command=lambda:ADD())
edit.pack(side='right', anchor='e')
search.pack(side='right', anchor='w',expand=True,padx=15,pady=5)
add = tb.Button(root, text='Add',takefocus=False,width=5,style=PRIMARY,command=lambda:EDIT())
