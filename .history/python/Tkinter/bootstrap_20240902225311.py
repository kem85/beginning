...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        style = tb.Style()
        style.configure('Custom.TCheckbutton', font=('Helvetica', 18,foreground='#C0C0C0',            # Text color (light gray)
    background='#110833' ))
        Catagory = tb.Checkbutton(addacc,width=20,text="Catagory",onvalue=1,offvalue=0,style='Custom.TCheckbutton').grid(column=1,row=4)
        Submit = tb.Button(addacc,takefocus=False,width=15,style=PRIMARY,text="Submit").grid(column=0,row=4)
def EDIT():
