...
def ADD():
    def clear(cond):
        if cond == 0:
            if len(color_var.get()) == 6 and name_var.get() != "" and ((email_var.get() != "" and password_var.get() !="" and Combo_Box.get() != "") or catagory_var.get())and color_var.get() != "":
                messagebox.showinfo(title="Successful", message="Added account successfully")
            else:
                print(catagory_var.get())
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Style = tb.Style()
        Style.configure('custom.TCombobox', foreground='#C0C0C0', font=('Helvetica', 18))
        Combo_Box = tb.Combobox(addacc,values=options,style='custom.TCombobox')
        style = tb.Style()
        style.configure('Custom.TCheckbutton', font=('Helvetica', 20),foreground='#C0C0C0')
        ...
        Submit = tb.Button(addacc,takefocus=False,width=10,style='Custom.TButton',text="Submit",command=lambda:(database(name_var.get(),email_var.get(),password_var.get(),color_var.get(),Combo_Box.get() or ""),clear(0)))
