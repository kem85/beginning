...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=15,style='Custom.TButton',text="Submit",command=lambda:database()).grid(column=0,row=4,pady=10,padx=10)
def EDIT():
...
def database(name,email,password,color,):
