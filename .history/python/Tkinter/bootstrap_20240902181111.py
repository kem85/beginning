...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        entry_font = ('Helvetica', 18)  # Customize font specifically for Entry
        entry = ttk.Entry(addacc, font=entry_font)
        entry.pack(pady=20)
        windowWidth = 450
        windowHeight = 300
        ...
        Name_Label = tb.Label(addacc, text='Name:',style=PRIMARY)
        Name_Label.grid(column=0,row=0)
        Password_Label = tb.Label(addacc, text='Password:',style=PRIMARY)
        Password_Label.grid(column=0,row=1)
        Name_Entry = tb.Entry(addacc,width=20).grid(column=1,row=0)
def EDIT():
