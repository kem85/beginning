...
def ADD():
    def clear(cond):
        if cond == 0:
            if len((color_var.get()) == 6 and color_var.get() != "") and name_var.get() != "" and ((email_var.get() != "" and password_var.get() !="" and Combo_Box.get() != "") or catagory_var.get()):
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=10,style='Custom.TButton',text="Submit",command=lambda:(database(name_var.get(),email_var.get(),password_var.get(),color_var.get() or "6e40c0",Combo_Box.get() or ""),clear(0)))
