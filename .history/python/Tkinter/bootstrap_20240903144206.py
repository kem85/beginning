...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=10,style='Custom.TButton',text="Submit",command=lambda:(database(name_var.get(),email_var.get(),password_var.get(),color_var.get() or "6e40c0",Combo_Box.get() or "off"),clear(0)))
