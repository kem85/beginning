...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=15,style='Custom.TButton',text="Submit",command=lambda:database(name,email,password,color,catagory)).grid(column=0,row=4,pady=10,padx=10)
def EDIT():
