...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Name_Label = tb.Label(addacc, text='Name:',style=PRIMARY,font=deffont,foreground="cyan").grid(column=0,row=0)
        Password_Label = tb.Label(addacc, text='Password:',style=PRIMARY,font=deffont,foreground="cyan").grid(column=0,row=1)
