...
def windowcreate(indic,update = False):
    ...
    if indic == 'catagory':
        if not update:
            ...
            button = tb.Button(second_frame, text=f'Button {poscat_var.get()+1}',takefocus=False,width=13,style='Custom.TButton')
            if poscat_var.get()+1 % 2 == 0:
                button.place(x=7, y=poscat_var.get()+1*35)
            else:
                button.place(x=180, y=(poscat_var.get())*35)
            poscat_var.set(i)
    else:
        pass
