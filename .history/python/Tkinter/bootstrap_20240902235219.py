...
buttons= tb.Style()
Name_Label = Email_Label = Password_Label = Color_Label = None
Name_Entry = Email_Entry = Password_Entry = Color_Entry = None
Catagory = None
...
def ADD():
    def clear(cond):
        if cond == 0:
            if len(color_var.get())<=6 and name_var.get() != "" and email_var.get() != "" and password_var.get() !="" and color_var.get() != "":
    ...
    global Name_Label, Email_Label, Password_Label, Color_Label
    global Name_Entry, Email_Entry, Password_Entry, Color_Entry
    global Catagory
    if not addwindow.get() and not editwindow.get():
        ...
        Name_Entry = tb.Entry(addacc,width=15,textvariable=name_var,font=deffont).grid(column=1,row=0,pady=10,padx=10)
        Email_Entry = tb.Entry(addacc,width=15,font=deffont,textvariable=email_var).grid(column=1,row=1,pady=10,padx=10)
        Password_Entry = tb.Entry(addacc,width=15,font=deffont,textvariable=password_var).grid(column=1,row=2,pady=10,padx=10)
        Color_Entry = tb.Entry(addacc,width=15,font=deffont,textvariable=color_var).grid(column=1,row=3,pady=10,padx=10)
        style = tb.Style()
        style.configure('Custom.TCheckbutton', font=('Helvetica', 18),foreground='#C0C0C0',command=lambda:clear(1))
        Catagory = tb.Checkbutton(addacc,width=15,text="Catagory",onvalue=1,offvalue=0,style='Custom.TCheckbutton',variable=catagory_var).grid(column=1,row=4,pady=10,padx=10)
        Submit = tb.Button(addacc,takefocus=False,width=15,style='Custom.TButton',text="Submit",command=lambda:(database(name_var.get(),email_var.get(),password_var.get(),color_var.get(),catagory_var.get()),clear(0))).grid(column=0,row=4,pady=10,padx=10)
def EDIT():
