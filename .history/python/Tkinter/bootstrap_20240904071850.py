...
def readbase(indic):
    if indic == "countc":
        cun.execute('''
                    SELECT COUNT(*) FROM database WHERE cata == 'false'
                    ''')
        greg = cun.fetchall()
        for i in cun:
            print(i)
...
second_frame = Frame(my_canvas, width=340, height=35*35) #50*45, 45 is y for each button and 50 is number of button
my_canvas.create_window((0, 0), window=second_frame, anchor="nw")
#####################################################################
print(readbase('buttonc'))
for i in range(35): #45 buttons
