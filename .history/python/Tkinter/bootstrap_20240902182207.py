...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Name_Label = tb.Label(addacc, text='Name:',style=PRIMARY,font=deffont,foreground="#7FFF00").grid(column=0,row=0)
        Password_Label = tb.Label(addacc, text='Password:',style=PRIMARY,font=deffont,foreground="#00BFFF").grid(column=0,row=1)
