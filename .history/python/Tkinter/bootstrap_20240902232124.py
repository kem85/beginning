...
from tkinter import BooleanVar, Toplevel, Frame, BOTH, LEFT, RIGHT, VERTICAL, Y,Messagebox
...
def ADD():
    def clear():
        mb = Messagebox.show_info("Submitted", "Account Has been Submitted")
        addwindow.set(False)
        addacc.destroy()
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=15,style='Custom.TButton',text="Submit",command=lambda:(database(name_var.get(),email_var.get(),password_var.get(),color_var.get(),catagory_var.get()),clear())).grid(column=0,row=4,pady=10,padx=10)
def EDIT():
