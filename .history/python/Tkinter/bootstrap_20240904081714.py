...
def windowcreate(indic,update = False):
    ...
    if indic == 'catagory':
            ...
            for i in range(readbase('countc')): #45 buttons
                buttonr = tb.Button(second_frame, text=f'Button {i}',takefocus=False,width=13,style='Custom.TButton')
                if i % 2 == 0:
                    buttonr.place(x=7, y=i*35)
                else:
                    buttonr.place(x=180, y=(i-1)*35)
                buttonss.append(buttonr)
            print(readbase('countc'))
    else:
        pass
