...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=10,style='Custom.TButton',text="Submit",command=lambda:(database(name_var.get(),email_var.get(),password_var.get(),color_var.get() or "6e40c0",Combo_Box.get() or "false"),clear(0),readbase("count")))
...
def database(name,email,password,color, catagory):
    ...
    sql = ''' INSERT INTO database(name,email,password,color,cata)
              VALUES(?,?,?,?,?) '''
    comm = [(name, email, password,color,catagory),]
    cun.executemany(sql, comm)
    conn.commit()

def readbase(indic):
    if indic == "count":
        data = cursor.execute("SELECT COUNT(*) FROM database")
        print(data)
