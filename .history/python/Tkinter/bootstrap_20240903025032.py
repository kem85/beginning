...
def ADD():
    def clear(cond):
        if cond == 0:
            if len(color_var.get()) == 6 and name_var.get() != "" and ((email_var.get() != "" and password_var.get() !="" and Combo_Box.get() != "" ) and catagory_var.get() != 1 )and color_var.get() != "":
                messagebox.showinfo(title="Successful", message="Added account successfully")
            else:
                print(Combo_Box.get(),catagory_var.get(),name_var.get(),password_var.get(),email_var.get(),color_var.get())
