...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        greg = ttk.Style()
        greg.configure('.', font=('Helvetica', 18))
        ...
        Name_Label = tb.Label(addacc, text='Name:',style=PRIMARY).grid(column=0,row=0)
        Password_Label = tb.Label(addacc, text='Password:',style=PRIMARY).grid(column=0,row=1)
        Name_Entry = tb.Entry(addacc,width=20,font=entry_font).grid(column=1,row=0)
def EDIT():
