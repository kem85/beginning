...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Email_Entry.grid(column=1,row=1,pady=5,padx=5)
        Password_Entry.grid(column=1,row=2,pady=5,padx=5)
        Color_Entry.grid(column=1,row=3,pady=5,padx=5)
        ...
        Submit.grid(column=1,row=4, sticky='w')
        # Combo_Box.grid(column=2,row=4,sticky='e')
def EDIT():
