...
def windowcreate(indic,update = False):
    ...
    if indic == 'catagory':
        if not update:
            ...
            buttonn = tb.Button(second_frame, text=f'Button {pos}',takefocus=False,width=13,style='Custom.TButton')
            if pos % 2 == 0:
                buttonn.place(x=7, y=pos*35)
            else:
                buttonn.place(x=180, y=(pos-1)*35)
            poscat_var.set(pos)
