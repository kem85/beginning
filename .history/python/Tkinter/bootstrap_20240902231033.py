...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=15,style='Custom.TButton',text="Submit",command=lambda:database(name_var.get(),email_var.get(),password_var.get(),color_var,catagory_var)).grid(column=0,row=4,pady=10,padx=10)
def EDIT():
