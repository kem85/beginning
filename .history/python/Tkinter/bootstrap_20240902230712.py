...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Name_Label = tb.Label(addacc, text='Name:',style=PRIMARY,variable=name_var,font=deffont,foreground="#C0C0C0").grid(column=0,row=0)
        Password_Label = tb.Label(addacc, text='Password:',style=PRIMARY,variable=var1,font=deffont,foreground="#C0C0C0").grid(column=0,row=2)
        Color_Label = tb.Label(addacc, text='Color:',style=PRIMARY,font=deffont,foreground="#C0C0C0").grid(column=0,row=3)
        Name_Entry = tb.Entry(addacc,width=15,font=deffont).grid(column=1,row=0,pady=10,padx=10)
        Email_Label = tb.Label(addacc, text='Email:',style=PRIMARY,font=deffont,foreground="#C0C0C0").grid(column=0,row=1)
        Email_Entry = tb.Entry(addacc,width=15,font=deffont).grid(column=1,row=1,pady=10,padx=10)
        Password_Entry = tb.Entry(addacc,width=15,font=deffont).grid(column=1,row=2,pady=10,padx=10)
        Color_Entry = tb.Entry(addacc,width=15,font=deffont).grid(column=1,row=3,pady=10,padx=10)
        style = tb.Style()
        style.configure('Custom.TCheckbutton', font=('Helvetica', 18),foreground='#C0C0C0')
        Catagory = tb.Checkbutton(addacc,width=15,text="Catagory",onvalue=1,offvalue=0,style='Custom.TCheckbutton').grid(column=1,row=4,pady=10,padx=10)
