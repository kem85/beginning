...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Submit = tb.Button(addacc,takefocus=False,width=10,style='Custom.TButton',text="Submit",command=lambda:(database(name_var.get(),email_var.get(),password_var.get(),color_var.get(),Combo_Box.get()),clear(0)))
...
def database(name,email,password,color,(catagory or ""):
