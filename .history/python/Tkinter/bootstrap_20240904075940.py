...
def windowcreate(indic,update = False):
    ...
    if indic == 'catagory':
        if not update:
            ...
            buttonn = tb.Button(second_frame, text=f'Button {poscat_var.get()}',takefocus=False,width=13,style='Custom.TButton')
            if poscat_var.get() % 2 == 0:
                buttonn.place(x=7, y=poscat_var.get()*35)
                print(poscat_var.get()*35)
            else:
                buttonn.place(x=180, y=(poscat_var.get()-1)*35)
                print(poscat_var.get()-1)*35)
            print(poscat_var.get())
    else:
        pass
