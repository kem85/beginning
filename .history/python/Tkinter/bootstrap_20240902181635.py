...
def ADD():
    ...
    if not addwindow.get() and not editwindow.get():
        ...
        Name_Entry = tb.Entry(addacc,width=20,font=deffont).grid(column=1,row=0,pady=10,padx=)
        Password_Entry = tb.Entry(addacc,width=20,font=deffont).grid(column=1,row=1)
def EDIT():
